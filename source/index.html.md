...
|-----|-----|-----|-----|
|2020.6.23 19:00|若干新增节点|新增|新增策略委托相关节点 |
...
|	source	|	string	|	TRUE	|		|	订单来源，默认api	|	api	|
...
|	errCode	|	integer	|	FALSE	|订单被拒状态码（仅对未成功创建的订单有效，如订单成功创建此字段不传）	|
|	errMessage }	|	string	|	FALSE	|订单被拒错误消息（仅对未成功创建的订单有效，如订单成功创建此字段不传）	|
...
|	sort	|	string	|	FALSE	|	desc	|	检索方向（以订单创建时间检索）	|asc - 由远及近, desc - 由近及远		|
...
注：<br>
•	orderStatus=triggered, 订单已成功触发；orderStatus=rejected, 订单触发失败；orderStatus=canceled, 订单在触发前被撤销；<br>
•	已成功触发的策略委托，应通过现货/杠杆交易相关接口查询其最新状态或撤销。策略委托一旦触发成功，通过策略委托历史接口获取的订单状态triggered为终态。<br>
