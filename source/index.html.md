...
1. 限制单连接每秒`有效`的上行数据限流(req，sub，unsub等)为50次/s（注：有效指的是合法的可以校验通过的数据格式，不包含ping/pong，此处秒限制为滑动窗口），当超过此限制时，会返回"too many request"错误消息。
2. 限制单API Key建连总数为：5，当超过此限制时，会返回"too many connection"错误消息。
3. 限制单IP建连数，100次/s，当超过次限制时，会返回"too many request"错误消息。
